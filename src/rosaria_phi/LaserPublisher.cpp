...
{
  assert(_l);
  ROS_INFO_NAMED("LaserPublisher", "LaserPublisher: initing");
  laser->lockDevice();
...
//    lasertf.setOrigin(tf::Vector3(laser->getSensorPositionX()/1000.0, laser->getSensorPositionY()/1000.0, laser->getSensorPositionZ()/1000.0));
    lasertf.setOrigin(tf::Vector3(laser->getSensorPositionX()/1000.0, laser->getSensorPositionY()/1000.0, 0));
...
//  laserscan.angle_max = ArMath::degToRad(laser->getEndDegrees());
  laserscan.angle_min = ArMath::degToRad(-90);
  laserscan.angle_max = ArMath::degToRad(90);
...
void LaserPublisher::publishLaserScan()
{
  ...
  const std::list<ArSensorReading*> *readings = laser->getRawReadings(); 
    ...
    std::vector < ArSensorReading > *readings2;
    std::vector < ArSensorReading > ::iterator it;
    ArPose p;
//    laser->lockDevice();
    readings2 = laser->getRawReadingsAsVector();
    it = readings2->begin();


  assert(readings);
//  ROS_INFO_NAMED("LaserPublisher", "LaserPublisher: laserscan: %lu readings\n", readings2->size());
